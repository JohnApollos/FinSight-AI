...
async def analyze_document(
    ...
    try:
        ...
        # Priority: UI-passed key, then fallback to .env key
        ...
        # Override sector with the user-selected sector
        extracted["sic"] = sector
        
        # 2. Financial Ratio & Solvency Anomaly Engine
        analysis = evaluate_financial_health(extracted)
        ...
        try:
            with mlflow.start_run():
                ...
                temp_json_path = os.path.join(TEMP_DIR, f"{file_id}.json")
        ...
        # Keep temp file for PDF compilation step, or clean up if needed
        # We clean up temp files older than 1 hour in a cleanup block
        pass
...
async def generate_report(
    ...
    """
    Ingests file, processes analysis, compiles the ReportLab PDF, 
    and streams it back as a download.
    """
    file_id = str(uuid.uuid4())
    _, ext = os.path.splitext(file.filename)
    temp_file_path = os.path.join(TEMP_DIR, f"{file_id}{ext}")
    pdf_output_path = os.path.join(TEMP_DIR, f"{file_id}.pdf")
    ...
    try:
        ...
        # Return PDF file as streaming download
        return FileResponse(
            pdf_output_path, 
            media_type="application/pdf", 
            filename=f"FinSight_Risk_Report_{extracted['company_name'].replace(' ', '_')}.pdf"
        )
        ...
        # Clean up files in background or in a delayed process
        # For this execution, we leave the files in temp so FileResponse can stream it.
        pass
